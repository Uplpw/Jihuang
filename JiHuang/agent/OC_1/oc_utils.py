...
class SoftmaxPolicy():
    ...
    def pmf(self, state, action_mask, action_mask_cpu):
        exponent = self.Q_U(state)
        exponent_copy = exponent.copy()
        exponent_copy_tensor = th.tensor(exponent_copy)
        mask_exponent = exponent_copy_tensor.masked_fill(action_mask_cpu, value=th.tensor(-10e3))
        # print("exponent:", mask_exponent)
        return np.exp(mask_exponent - logsumexp(mask_exponent))

    def sample(self, state, action_mask, action_mask_cpu):
        # print("------------------------------")
        p = self.pmf(state, action_mask, action_mask_cpu)
        # print("self.pmf(state):", p.numpy()[0])
        return int(self.rng.choice(self.nactions, p=p.numpy()[0]))

    def gradient(self):
        pass

    def update(self, state, action, Q_U, action_mask1, action_mask_cpu1):
        actions_pmf = self.pmf(state, action_mask1, action_mask_cpu1).numpy()[0]
